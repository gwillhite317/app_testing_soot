...
def session_from_credentials(username: str, password: str) -> requests.Session:
    ...
    # ── Step 1: Hit ASDC auth endpoint and capture the first redirect ──────
    r1 = session.get(AUTH_URL, allow_redirects=False, timeout=30)

    if r1.status_code not in (301, 302, 303, 307, 308):
        raise RuntimeError(
            f"Unexpected auth response status: {r1.status_code}. "
            "The ASDC auth endpoint may have changed."
        )

    auth_redirect_url = r1.headers.get("Location", "")
    if not auth_redirect_url or "urs.earthdata.nasa.gov" not in auth_redirect_url:
        raise RuntimeError(
            f"Unexpected redirect target: {auth_redirect_url or r1.url}. "
            "The ASDC auth endpoint may have changed."
        )

    # Follow the redirect, but do not auto-follow the full chain yet.
    r2 = session.get(auth_redirect_url, allow_redirects=False, timeout=30)

    if r2.status_code in (301, 302, 303, 307, 308):
        next_location = r2.headers.get("Location", "")

        # Current ASDC/URS behavior can sometimes skip the interactive login page
        # and immediately issue a login-code redirect back to ASDC.
        if next_location.startswith(f"{BASE_URL}/login") and "code=" in next_location:
            r3 = session.get(next_location, allow_redirects=False, timeout=30)
            if r3.status_code in (301, 302, 303, 307, 308):
                follow_location = r3.headers.get("Location")
                if follow_location:
                    session.get(follow_location, allow_redirects=False, timeout=30)
            oauth_params = _extract_oauth_params(auth_redirect_url)
            login_payload = None
        else:
            # If URS returns the login page, parse it and post credentials.
            r3 = session.get(auth_redirect_url, allow_redirects=True, timeout=30)
            authenticity_token = _extract_authenticity_token(r3.text)
        ...
        raise RuntimeError(
            f"Unexpected response from URS authorize endpoint: {r2.status_code}. "
